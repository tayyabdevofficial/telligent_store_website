<x-layouts::app :title="__('Payments')">
    <div class="space-y-6 p-4 md:p-8">
        <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
                <h1 class="text-3xl font-extrabold tracking-tight text-slate-950">Payments</h1>
                <p class="mt-2 text-sm text-slate-600">Stripe payment records are persisted and visible here for audit and support work.</p>
            </div>
            <a href="{{ route('admin.payment-links.index') }}" class="rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-sky-600">Payment links</a>
        </div>
        ...
        <form method="GET" class="admin-card p-4 md:p-5">
            <div class="grid gap-4 md:grid-cols-[1.4fr_1fr_1fr_1fr_auto_auto] md:items-end">
                ...
                <a href="{{ route('admin.payments.index') }}" class="rounded-full border border-slate-300 px-5 py-3 text-center text-sm font-semibold text-slate-700 transition hover:border-slate-950 hover:text-slate-950">Reset</a>
            </div>
        </form>
            ...
            <table class="min-w-full w-full divide-y divide-slate-200 text-sm">
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    @foreach ($payments as $payment)
                        <tr>
                            <td class="px-6 py-4">
                                <p class="font-semibold text-slate-900">{{ strtoupper(str_replace('_', ' ', $payment->provider)) }}</p>
                                <p class="text-xs text-slate-400">{{ $payment->order?->paymentLink?->title ?: ($payment->order?->custom_title ?: ($payment->order?->package?->name ?: 'Standard order')) }}</p>
                            </td>
                            <td class="px-6 py-4 text-slate-600">{{ $payment->customer_email ?: 'N/A' }}</td>
                            ...
                            <td class="px-6 py-4 text-slate-600">${{ number_format($payment->amount, 2) }}</td>
                            <td class="px-6 py-4 text-xs text-slate-600">
                                @php($paymentDate = $payment->paid_at ?: $payment->created_at)
                                <p class="whitespace-nowrap">US: {{ $paymentDate?->copy()->timezone('America/New_York')->format('M d, Y h:i A') ?: 'N/A' }}</p>
                                <p class="mt-1 whitespace-nowrap">PK: {{ $paymentDate?->copy()->timezone('Asia/Karachi')->format('M d, Y h:i A') ?: 'N/A' }}</p>
                            </td>
                            <td class="px-6 py-4 text-right"><a href="{{ route('admin.payments.show', $payment) }}" class="font-semibold text-sky-600">Open</a></td>
                        </tr>
                    @endforeach
                    @if ($payments->isEmpty())
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-sm text-slate-500">No payments found for the selected filters.</td>
                        </tr>
                    @endif
                </tbody>
            </table>
